...
from ugp.modules.frequency_spatial import FrequencySpatialAdaptiveAttention
...
class ProgressiveGeoTransformer(nn.Module):
    def __init__(
        ...
        Args:
            ...
            use_gating: whether to use adaptive gating fusion in FSAA
        """
        ...
        # Add Frequency-Spatial Adaptive Attention before the main transformer blocks
        # The input channels to FSAA is 128 (points) + 64 (BEV) = 192 (which is hidden_dim)
        # Reduced Chebyshev K from 3 to 2 to prevent overfitting
        self.fsaa = FrequencySpatialAdaptiveAttention(in_channels=hidden_dim, K=2, use_gating=use_gating)
    ...
    def forward(
        ...
        # Apply Frequency-Spatial Adaptive Attention
        # Shape of ref_points/src_points is (B, N, 3), features is (B, N, 192)
        ref_fuse_feats = self.fsaa(ref_points, ref_fuse_feats)
        src_fuse_feats = self.fsaa(src_points, src_fuse_feats)
